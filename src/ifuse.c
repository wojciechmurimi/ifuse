...
// #include "C:\\Users\\murim\\codes\\test\\fuse3\\fuse.h"
#include <fuse.h>
#include <errno.h>
#include <fcntl.h>
#include <stdint.h>
...
// mhot" Stop-Process -Name ifuse -Force
...
#ifdef WIN32
#define PACKAGE_NAME "ifuse-win"
#define PACKAGE_VERSION "1.0.0"
#define PACKAGE_URL "github.com/wojciechmurimi/ifuse"
#define PACKAGE_BUGREPORT "github.com/wojciechmurimi/ifuse"
#endif
...
struct afc_error_mapping {
  afc_error_t from;
  int to;
} static afc_error_to_errno_map[] = {{AFC_E_SUCCESS, 0},
...
static int ifuse_getattr(const char *path, struct fuse_stat *stbuf,
  ...
  const char *s_ifmt =
      plist_get_string_ptr(plist_dict_get_item(info, "st_ifmt"), NULL);
  if (s_ifmt) {
...
#ifndef WIN32
  stbuf->st_mtime =
      (time_t)(plist_dict_get_uint(info, "st_mtime") / 1000000000);
#endif
#ifdef _DARWIN_FEATURE_64_BIT_INODE
  /* available on iOS 7+ */
  stbuf->st_birthtime =
      (time_t)(plist_dict_get_uint(info, "st_birthtime") / 1000000000);
#endif
  plist_free(info);
  ...
  struct fuse_context *ctx = dyn_fuse_get_context();
...
static int ifuse_utimens(const char *path, const struct fuse_timespec tv[2],
  ...
  if (err == AFC_E_UNKNOWN_PACKET_TYPE) {
    /* ignore error for pre-3.1 devices as they do not support setting file
     * modification times */
    return 0;
  }
  if (err != AFC_E_SUCCESS) {
...
static int ifuse_fsync(const char *path, int datasync,
                       struct fuse_file_info *fi) {
  return 0;
}
...
int ifuse_flush(const char *path, struct fuse_file_info *fi) { return 0; }

int ifuse_chmod(const char *path, fuse_mode_t mode, struct fuse_file_info *fi) {
  return 0;
}

int ifuse_chown(const char *file, fuse_uid_t user, fuse_gid_t group,
                struct fuse_file_info *fi) {
  return 0;
}
...
static struct fuse_operations ifuse_oper = {.getattr = ifuse_getattr,
                                            .statfs = ifuse_statfs,
...
static void print_usage() {
  ...
  fprintf(stderr, "  --documents APPID\tmount 'Documents' folder of app "
                  "identified by APPID\n");
  fprintf(stderr, "  --container APPID\tmount sandbox root of an app "
                  "identified by APPID\n");
  fprintf(
      stderr,
...
static void list_available_apps(idevice_t dev) {
  ...
  if (instproxy_client_start_service(dev, &ip, "ifuse") !=
      INSTPROXY_E_SUCCESS) {
    fprintf(stderr,
            "ERROR: Couldn't connect to installation proxy on device\n");
    goto leave_cleanup;
  }
  ...
  instproxy_client_options_set_return_attributes(
      client_opts, "CFBundleIdentifier", "CFBundleDisplayName",
      "CFBundleVersion", "UIFileSharingEnabled", NULL);
...
#ifdef WIN32
void *resolve_winfsp_func(HMODULE hmodule, const char *name) {
  void *dyn = GetProcAddress(hmodule, name);

  if (dyn == NULL) {
    printf("FUNCTION RESOLVE FAIL: %s\n", name);
    exit(1);
  }
...
int main(int argc, char *argv[]) {
  ...
  HMODULE hmodule =
      ...
      LoadLibrary("C:\\Program Files (x86)\\WinFsp\\bin\\winfsp-a64.dll");
    #elif defined(__i386__)
      LoadLibrary("C:\\Program Files (x86)\\WinFsp\\bin\\winfsp-x86.dll");
    #else
      #error("Unsupported arch")
    #endif

  if (hmodule == NULL) {
    printf("Failed to load WinFsp. Make sure Winfsp is installed at C:\\Program Files (x86)\\WinFsp\\");
    exit(1);
  }

  dyn_fuse_get_context = resolve_winfsp_func(hmodule, "fuse_get_context");
  typeof(&fuse_opt_parse) dyn_fuse_opt_parse =
      resolve_winfsp_func(hmodule, "fuse_opt_parse");
  typeof(&fuse_opt_add_arg) dyn_fuse_opt_add_arg =
      resolve_winfsp_func(hmodule, "fuse_opt_add_arg");
  typeof(&fuse3_main_real) dyn_fuse_main_real =
      resolve_winfsp_func(hmodule, "fuse3_main_real");
#else
...
#define dyn_fuse_main_real fuse_main_real
  ...
  enum idevice_options lookup_opts =
      IDEVICE_LOOKUP_USBMUX | IDEVICE_LOOKUP_NETWORK;

  memset(&opts, 0, sizeof(opts));
  opts.service_name = AFC_SERVICE_NAME;

  if (dyn_fuse_opt_parse(&args, NULL, ifuse_opts, ifuse_opt_proc) == -1) {
    return EXIT_FAILURE;
  }
  ...
  err = idevice_new_with_options(&device, opts.device_udid,
                                 (opts.use_network) ? IDEVICE_LOOKUP_NETWORK
  ...
  if (err != IDEVICE_E_SUCCESS) {
    ...
    fprintf(stderr, "Ensure that the device is connected and usbmuxd is "
                    "properly installed.\n");
    return EXIT_FAILURE;
  }
  ...
  if (ret != LOCKDOWN_E_SUCCESS) {
    ...
    if (ret == LOCKDOWN_E_PASSWORD_PROTECTED) {
      fprintf(stderr, "Please disable the password protection on your device "
                      "and try again.\n");
      fprintf(stderr, "The device does not allow pairing as long as a password "
                      "has been set.\n");
      fprintf(stderr,
      ...
      fprintf(stderr, "The device does not allow pairing as long as the dialog "
  ...
  if ((lockdownd_start_service(control, opts.service_name, &opts.service) !=
       LOCKDOWN_E_SUCCESS) ||
      !opts.service) {
    lockdownd_client_free(control);
    idevice_free(device);
    ...
    if (!strcmp(opts.service_name, AFC2_SERVICE_NAME)) {
      fprintf(stderr, "This service enables access to the root filesystem of "
                      "your device.\n");
      fprintf(stderr, "Your device needs to be jailbroken and have the AFC2 "
                      "service installed.\n");
    }
    return EXIT_FAILURE;
  }
  ...
  if (!strcmp(opts.service_name, HOUSE_ARREST_SERVICE_NAME)) {
    ...
    /* FIXME: iOS 3.x house_arrest does not know about VendDocuments yet, thus
     * use VendContainer and chroot manually with fuse subdir module */
    if (house_arrest_send_command(house_arrest,
                                  opts.use_container ? "VendContainer"
                                                     : "VendDocuments",
                                  opts.appid) != HOUSE_ARREST_E_SUCCESS) {
      fprintf(stderr, "Could not send house_arrest command!\n");
      goto leave_err;
    }

    plist_t dict = NULL;
    if (house_arrest_get_result(house_arrest, &dict) !=
    ...
    if (node) {
      ...
      if (str && !strcmp(str, "InstallationLookupFailed")) {
        fprintf(stderr,
                "The App '%s' is either not present on the device, or the "
                "'UIFileSharingEnabled' key is not set in its Info.plist. "
                "Starting with iOS 8.3 this key is mandatory to allow access "
                "to an app's Documents folder.\n",
                opts.appid);
      }
      free(str);
      goto leave_err;
